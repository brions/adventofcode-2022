...
class Coordinates3D(Coordinates):
    """Represents a 3D coordinate pair (x,y,z) that is comparable in a given
    dimension, scriptable, hashable, and can determine adjacency with another
    Coordinates3D object.
    """
    ...
    def __ne__(self, other: "Coordinates3D") -> bool:
        if other is None:
            return True
        return (self.x, self.y, self.z) != (other.x, other.y, other.z)
    ...
    def compareZ(self, other: "Coordinates3D"):
        return self.z - other.z
    
    def isAdjacent(self, other: "Coordinates3D", dir: Direction = Direction.ANY) -> bool:
        """Determines if the other Coordinates3D is adjacent to this in any face.

        Args:
            other (Coordinates3D): Coordinate3D to compare to this
            dir (Direction): face to compare (up/down == top/bottom respectively)
        ...
        if dir == Direction.ANY:
            ...
            #    other == Coordinates(self['x']-1, self['y']):
            #     return True
            # if other == Coordinates3D(self['z'], self['z']+1) or \
            #    other == Coordinates3D(self['z'], self['z']-1):
            #     return True
        elif dir == Direction.DOWN:
            if other == Coordinates3D(self['x'], self['y'] - 1, self['z']):
                return True
        elif dir == Direction.UP:
            if other == Coordinates3D(self['x'], self['y'] + 1, self['z']):
                return True
        elif dir == Direction.RIGHT:
            if other == Coordinates3D(self['x'] + 1, self['y'], self['z']):
                return True
        elif dir == Direction.LEFT:
            if other == Coordinates3D(self['x'] -1, self['y'], self['z']):
                return True
        elif dir == Direction.FRONT:
            if other == Coordinates3D(self['x'], self['y'], self['z'] + 1):
                return True
        elif dir == Direction.BEHIND:
            if other == Coordinates3D(self['x'], self['y'], self['z'] - 1):
                return True
